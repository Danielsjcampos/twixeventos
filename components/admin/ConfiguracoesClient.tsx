...
const TABS: Tab[] = [
  {
    ...
    label: 'Geral',
    icon: Globe,
    description: 'Identidade visual e dados básicos do site',
  },
  {
    id: 'seo',
    label: 'SEO',
    icon: Search,
    description: 'Meta tags, títulos e indexação para buscadores',
  },
  {
    id: 'integracoes',
    label: 'Integrações',
    icon: Zap,
    description: 'Analytics, Pixel e motor de IA/busca',
  },
  {
    id: 'whatsapp',
    label: 'WhatsApp / API',
    icon: MessageCircle,
    description: 'Número, mensagem padrão e integração 0API',
  },
  {
    id: 'hero',
    label: 'Hero / Slides',
    icon: Layers,
    description: 'Slides do banner principal da home — imagens, vídeos ou gradientes',
  },
  {
    id: 'conteudo',
    label: 'Conteúdo',
    icon: Layout,
    description: 'Vídeo, banners e textos do site público',
  },
  {
    id: 'social',
    label: 'Redes Sociais',
    icon: Share2,
    description: 'Links das redes sociais exibidos no footer',
  },
  {
    id: 'automacoes',
    label: 'Automações',
    icon: Bot,
    description: 'WhatsApp automático: aniversários, pesquisas e avaliações',
  },
  {
    id: 'sitemap',
    label: 'Sitemap / GSC',
    icon: Map,
    description: 'Sitemap dinâmico e indexação no Google Search Console',
  },
  {
    id: 'cashback',
    label: 'Área do Cliente',
    icon: Gift,
    description: 'Programa de cashback e configurações do portal do cliente',
  },
  {
    id: 'sistema',
    label: 'Sistema',
    icon: Settings,
    description: 'SLA, alertas e parâmetros operacionais',
  },
]
  ...
  return (
    <div className="space-y-3">
      ...
      {premios.map((p) => (
        <div
          key={p.id}
          className="bg-brand-surface-2 border border-brand-border rounded-xl overflow-hidden"
        >
          {/* Header row */}
          <div
          ...
          >
            ...
            />
            <span className="font-medium text-brand-text text-sm flex-1 truncate">
              {p.nome || 'Sem nome'}
            </span>
            ...
            <ChevronRight
              className={cn(
                'size-4 text-brand-muted transition-transform',
                editId === p.id && 'rotate-90'
              )}
            />
            <button
              type="button"
          ...
          {editId === p.id && (
            <div className="border-t border-brand-border px-4 py-4 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    value={p.peso}
                    onChange={(e) =>
                      updatePremio(p.id, 'peso', Math.max(1, parseInt(e.target.value) || 1))
                    }
                    className="w-full bg-brand-surface border border-brand-border rounded-lg px-3 py-2 text-brand-text text-sm focus:outline-none focus:border-brand-accent transition-colors"
                  />
                  ...
                  {p.tipo === 'fixo' && (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-xs text-emerald-400 whitespace-nowrap">
                        → credita R$ {(p.valorCredito ?? 0).toFixed(2)}
                      </span>
                    </div>
                  )}
                  {p.tipo === 'percentual' && (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <span className="text-brand-muted text-sm">
                        % do cashback total acumulado
                      </span>
                    </div>
                  )}
                  {(!p.tipo || (p.tipo === 'fixo' && (p.valorCredito ?? 0) === 0)) && (
                    <p className="text-xs text-brand-muted mt-1">
                      📞 O cliente precisará contatar via WhatsApp para resgatar.
                    </p>
                  )}
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="block text-xs font-medium text-brand-muted">
                  Descrição (exibida no modal ao ganhar)
                </label>
                <input
                  type="text"
                  value={p.descricao}
        ...
        <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-3 text-xs text-brand-muted">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {premios.map((p) => (
              <span
                key={p.id}
                className="flex items-center gap-1.5 bg-brand-surface rounded-full px-2.5 py-1"
              >
...
export function ConfiguracoesClient({ initialConfigs }: { initialConfigs: ConfigMap }) {
  ...
  const [indexarStatus, setIndexarStatus] = useState<null | {
    ok: boolean
  ...
  const tabs: Record<string, { keys: string[]; content: React.ReactNode }> = {
    ...
    geral: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="Informações básicas exibidas no site e nos buscadores">
            Identidade do Site
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Nome do site" required>
              <Input
                value={get('site_nome')}
                onChange={set('site_nome')}
                placeholder="Twix Eventos"
              />
            </Field>
            <Field label="Telefone / WhatsApp" required>
              <Input
                value={get('site_telefone')}
                onChange={set('site_telefone')}
                placeholder="5512996498725"
                mono
              />
            </Field>
          </div>
          <Field label="Descrição curta" hint="Usada como meta description padrão">
            <Textarea
              value={get('site_descricao')}
              onChange={set('site_descricao')}
              placeholder="Locação de brinquedos infláveis em São José dos Campos…"
            />
          </Field>
          <Field label="Endereço completo">
            <Input
              value={get('site_endereco')}
              onChange={set('site_endereco')}
              placeholder="R. Prof. Roberval Fróes, 390 – SJC/SP"
            />
          </Field>

          <Divider />
          <SectionTitle description="Faça upload das imagens principais da sua marca">
            Imagens e Mídia
          </SectionTitle>
          ...
          <Field
            label="Imagem de preview social (OG Image)"
            hint="Exibida quando o link é compartilhado no WhatsApp, Instagram, etc. Ideal: 1200×630 px"
          >
            <SingleImageUpload
    ...
    seo: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="Configure como o site aparece nos resultados de busca">
            Meta Tags e Indexação
          </SectionTitle>
          <Field
            label="Título padrão"
            hint="Exibido na aba do navegador e no Google. Máx. 60 caracteres"
          >
            <Input
              value={get('seo_titulo_padrao')}
              onChange={set('seo_titulo_padrao')}
              placeholder="Twix Eventos | Locação de Brinquedos em São José dos Campos"
            />
            <p className="text-xs text-brand-muted text-right mt-1">
              {get('seo_titulo_padrao').length}/60 chars
            </p>
          </Field>
          <Field
            label="Template de título por página"
            hint="Use %s para o título da página. Ex: %s | Twix Eventos"
          >
            <Input
              value={get('seo_template_titulo')}
              onChange={set('seo_template_titulo')}
              placeholder="%s | Twix Eventos"
              mono
            />
          </Field>
          <Field
            label="Descrição padrão"
            hint="Máx. 160 caracteres. Aparece abaixo do título no Google"
          >
            <Textarea
              value={get('seo_descricao_padrao')}
              onChange={set('seo_descricao_padrao')}
              placeholder="Aluguel de brinquedos infláveis para festas em SJC…"
            />
            <p className="text-xs text-brand-muted text-right mt-1">
              {get('seo_descricao_padrao').length}/160 chars
            </p>
          </Field>
          <Field
            label="Palavras-chave"
            hint="Separadas por vírgula. Ex: brinquedos infláveis, locação SJC, festa infantil"
          >
            <Textarea
              value={get('seo_palavras_chave')}
              onChange={set('seo_palavras_chave')}
              placeholder="brinquedos infláveis, locação, São José dos Campos…"
              rows={2}
            />
          </Field>

          <Divider />
          <SectionTitle description="Verificação de propriedade e controle de indexação">
            Verificação e Robôs
          </SectionTitle>
          <Field
            label="Google Search Console — código de verificação"
            hint="Apenas o valor do atributo content da meta tag"
          >
            <Input
          ...
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            ...
            <Toggle
              value={get('robots_indexar') || 'true'}
              onChange={set('robots_indexar')}
    ...
    integracoes: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="IDs de rastreamento para análise de tráfego e conversões">
            Analytics e Rastreamento
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Google Analytics 4 — Measurement ID" hint="Formato: G-XXXXXXXXXX">
              <Input
                value={get('google_analytics_id')}
            ...
            <Field label="Google Tag Manager — Container ID" hint="Formato: GTM-XXXXXXX">
              <Input
                value={get('google_tag_manager_id')}
          ...
          <Divider />
          <SectionTitle description="Motor de inteligência artificial usado para busca e atendimento no site">
            IA / Motor de Busca
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field
              label="Nome do provedor de IA"
              hint="Ex: OpenAI, Anthropic, Groq, Gemini, DeepSeek"
            >
              <Input
                value={get('ia_motor_nome')}
                onChange={set('ia_motor_nome')}
                placeholder="OpenAI"
              />
            </Field>
          ...
          <Field label="API Key do provedor de IA">
            <SecretInput
              value={get('ia_api_key')}
              onChange={set('ia_api_key')}
              placeholder="sk-…"
            />
          </Field>
        </div>
    ...
    whatsapp: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="Número e mensagem usados nos botões de contato do site">
            WhatsApp Público
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field
              label="Número do WhatsApp"
              hint="Formato internacional sem + ou espaços: 5512999990000"
              required
            >
              <Input
                value={get('whatsapp_numero')}
                onChange={set('whatsapp_numero')}
                placeholder="5512996498725"
                mono
              />
            </Field>
            <div className="flex items-center gap-3 pt-6">
              <Toggle
                value={get('whatsapp_ativo') || 'true'}
                onChange={set('whatsapp_ativo')}
          ...
          <Field label="Mensagem padrão" hint="Texto pré-preenchido ao abrir o WhatsApp">
            <Textarea
              value={get('whatsapp_mensagem_padrao')}
              onChange={set('whatsapp_mensagem_padrao')}
              placeholder="Olá! Gostaria de reservar um brinquedo para meu evento."
              rows={2}
            />
          </Field>

          <Divider />
          <SectionTitle description="Integração com a plataforma 0API para automações e notificações via WhatsApp">
            Integração 0API / Z-API
          </SectionTitle>
          <div className="p-3 bg-blue-500/8 border border-blue-500/20 rounded-lg text-xs text-blue-600 dark:text-blue-400 mb-4">
            Estas credenciais são usadas para envio automático de confirmações, lembretes de eventos
            e follow-ups de leads pelo WhatsApp.
          </div>
          <Field
            label="URL base da API"
            hint="Ex: https://api.z-api.io/instances/SEU_ID/token/SEU_TOKEN"
          >
            <Input
              value={get('zapi_url')}
              onChange={set('zapi_url')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Instance ID">
              <Input
                value={get('zapi_instance_id')}
                onChange={set('zapi_instance_id')}
                placeholder="3ABC123…"
                mono
              />
            </Field>
            <Field label="Token de acesso">
              <SecretInput
                value={get('zapi_token')}
                onChange={set('zapi_token')}
          ...
          <Field label="Client-Token (header Security)">
            <SecretInput
              value={get('zapi_client_token')}
              onChange={set('zapi_client_token')}
              placeholder="F…"
            />
          </Field>
        </div>
    ...
    conteudo: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="Vídeo exibido na seção de apresentação da página inicial">
            Vídeo de Apresentação
          </SectionTitle>
          <Field
            label="URL do vídeo YouTube"
            hint="Aceita: youtube.com/watch?v=ID, youtu.be/ID ou apenas o ID do vídeo"
          >
            <Input
              value={get('video_apresentacao')}
              onChange={set('video_apresentacao')}
          ...
          <Divider />
          <SectionTitle description="Banner de oferta exibido no topo de todas as páginas">
            Banner Promocional
          </SectionTitle>
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            <div>
            ...
            <Toggle
              value={get('banner_ativo') || 'true'}
              onChange={set('banner_ativo')}
              label="Banner ativo"
            />
          </div>
          <Field label="Texto do banner" hint="Texto exibido na faixa promocional">
            <Input
              value={get('banner_texto')}
              onChange={set('banner_texto')}
              placeholder="Descontos especiais de segunda a quinta! Reserve agora →"
            />
          </Field>
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            <div>
              ...
              <p className="text-brand-muted text-xs">
                Ativa destaque de preço reduzido nos dias úteis
              </p>
            </div>
            <Toggle
              value={get('desconto_seg_qui') || 'false'}
              onChange={set('desconto_seg_qui')}
              label="Desconto Seg-Qui"
            />
          </div>

          <Divider />
          <SectionTitle description="Textos da seção principal (Hero) da página inicial">
            Hero da Página Inicial
          </SectionTitle>
          <Field label="Título principal" hint="Texto grande em destaque">
            <Input
              value={get('hero_titulo')}
              onChange={set('hero_titulo')}
              placeholder="DIVERSÃO GARANTIDA PARA O SEU EVENTO"
            />
          </Field>
          <Field label="Subtítulo / descrição">
            <Textarea
              value={get('hero_subtitulo')}
              onChange={set('hero_subtitulo')}
              placeholder="Mais de 24 brinquedos infláveis e eletrônicos para festas…"
              rows={2}
            />
          </Field>
        </div>
    ...
    social: {
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="URLs completas dos perfis. Deixe em branco para ocultar o ícone no footer.">
          ...
          <Field label="Instagram">
            <Input
              ...
              placeholder="https://instagram.com/twixeventos"
            />
          </Field>
          <Field label="Facebook">
            <Input
              ...
              placeholder="https://facebook.com/twixeventos"
            />
          </Field>
          <Field label="YouTube">
            <Input
              ...
              placeholder="https://youtube.com/@twixeventos"
            />
          </Field>
          <Field label="TikTok">
            <Input
              ...
              placeholder="https://tiktok.com/@twixeventos"
            />
          </Field>
          <Field
            label="Google Maps (link da empresa)"
            hint="Link do Google Maps para a localização da empresa"
          >
            <Input
              value={get('social_google_maps')}
              onChange={set('social_google_maps')}
    ...
    automacoes: {
      keys: [
        ...
        'whatsapp_instance',
        'whatsapp_ativo',
        'aniversario_dias_antes',
        'aniversario_mensagem',
        'pesquisa_ativo',
        'pesquisa_horas_apos',
        'pesquisa_mensagem',
        'pesquisa_nota_minima',
        'google_review_link',
      ],
      content: (
        <div className="space-y-5">
          <SectionTitle description="Configure a API do WhatsApp para envio automático de mensagens">
            Conexão API WhatsApp
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Ativar automação WhatsApp">
              <Toggle
                value={get('whatsapp_ativo') || 'false'}
                onChange={set('whatsapp_ativo')}
            ...
            <Field label="URL da API" hint="Ex: https://api.0api.com ou similar">
              <Input
                value={get('whatsapp_api_url')}
                onChange={set('whatsapp_api_url')}
                placeholder="https://api.0api.com"
                mono
              />
            </Field>
            <Field label="Instance ID">
              <Input
                value={get('whatsapp_instance')}
                onChange={set('whatsapp_instance')}
          ...
          <Divider />
          <SectionTitle description="Mensagem automática de parabéns para aniversários de clientes">
            🎂 Mensagens de Aniversário
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field
              label="Dias de antecedência"
              hint="Quantos dias antes do aniversário enviar a mensagem"
            >
              <Input
                value={get('aniversario_dias_antes')}
                onChange={set('aniversario_dias_antes')}
                placeholder="3"
                type="number"
              />
            </Field>
          ...
          <Field
            ...
            hint="Use {nome} para o nome do cliente e {aniversariante} para o nome do aniversariante"
          >
            <Textarea
              value={get('aniversario_mensagem')}
              onChange={set('aniversario_mensagem')}
              rows={4}
          ...
          <Divider />
          <SectionTitle description="Pesquisa automática enviada após realização dos eventos">
            ⭐ Pesquisa Pós-Evento
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Ativar pesquisa pós-evento">
              <Toggle
                value={get('pesquisa_ativo') || 'false'}
                onChange={set('pesquisa_ativo')}
                label="Pesquisa ativa"
              />
            </Field>
            <Field label="Horas após o evento" hint="Quantas horas após o evento enviar a pesquisa">
              <Input
                value={get('pesquisa_horas_apos')}
                onChange={set('pesquisa_horas_apos')}
                placeholder="24"
                type="number"
              />
            </Field>
            <Field
              label="Nota mínima para Google Review"
              hint="Se a resposta for maior ou igual, envia link do Google"
            >
              <input
                type="number"
                min="1"
                max="5"
                value={get('pesquisa_nota_minima')}
                onChange={(e) => set('pesquisa_nota_minima')(e.target.value)}
                placeholder="4"
            ...
            <Field label="Link do Google Meu Negócio">
              <Input
                value={get('google_review_link')}
                onChange={set('google_review_link')}
          ...
          <Field label="Mensagem da pesquisa" hint="Use {nome} para o nome do cliente">
            <Textarea
              value={get('pesquisa_mensagem')}
              onChange={set('pesquisa_mensagem')}
              rows={4}
          ...
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 text-sm text-brand-muted">
            ...
            <ul className="space-y-1 text-xs mt-2">
              <li>
                <code className="text-blue-400">{'{nome}'}</code> — Nome do cliente
              </li>
              <li>
                <code className="text-blue-400">{'{aniversariante}'}</code> — Nome do aniversariante
                (apenas em mensagens de aniversário)
              </li>
              <li>
                <code className="text-blue-400">{'{google_link}'}</code> — Link do Google Meu
                Negócio (apenas na pesquisa)
              </li>
            </ul>
          </div>
        </div>
    ...
    sitemap: {
      ...
      content: (() => {
        ...
        return (
          <div className="space-y-5">
            <SectionTitle description="URL canônica do site usada no sitemap e nas notificações ao Google">
              URL do Site
            </SectionTitle>
            <Field
              label="URL base do site"
              hint="Ex: https://twixeventos.com (sem barra no final)"
              required
            >
              <Input
                value={get('site_url')}
                onChange={set('site_url')}
                placeholder="https://twixeventos.com"
                mono
              />
            </Field>
            ...
            <Divider />
            <SectionTitle description="Controle a frequência e prioridade das URLs no sitemap.xml">
              Configurações do Sitemap
            </SectionTitle>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <Field
                label="Frequência de atualização"
                hint="Indica ao Google com que frequência o conteúdo muda"
              >
                <select
              ...
              <Field
                ...
                hint="Valor entre 0.0 e 1.0 para páginas de brinquedos"
              >
                <select
            ...
            <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
              ...
              <Toggle
                value={get('sitemap_include_blog') || 'false'}
                onChange={set('sitemap_include_blog')}
                label="Blog no sitemap"
              />
            </div>

            <Divider />
            <SectionTitle description="Credenciais do Google Search Console para indexação automática via Indexing API">
              Google Search Console
            </SectionTitle>
            <Field
              label="Google Site Verification (Meta Tag de Verificação)"
              hint="Cole a meta tag HTML completa (ex: <meta name='google-site-verification' content='...' />) ou apenas o código content (ex: Zmg1U2V9Mx7TMA5wzJa2iQOBHIxTzSx9qxc-RuBK4FQ)."
            >
              <Input
                value={get('google_site_verification')}
                onChange={set('google_site_verification')}
                placeholder="Zmg1U2V9Mx7TMA5wzJa2iQOBHIxTzSx9qxc-RuBK4FQ"
                mono
              />
            </Field>
            <Field
              label="URL cadastrada no GSC"
              hint="Deve corresponder exatamente à URL cadastrada no Search Console"
            >
              <Input
                value={get('gsc_site_url')}
                onChange={set('gsc_site_url')}
                placeholder="https://twixeventos.com"
                mono
              />
            </Field>
            ...
            <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
              <div>
                ...
                <p className="text-brand-muted text-xs">
                  Notifica o Google automaticamente quando um brinquedo é criado ou editado
                </p>
              </div>
              <Toggle
                value={get('indexar_automatico') || 'false'}
                onChange={set('indexar_automatico')}
                label="Indexação automática"
              />
            </div>

            <Divider />
            <SectionTitle description="Envie todas as URLs do sitemap para o Google agora">
              Indexar Agora
            </SectionTitle>
                  ...
                  <div className="space-y-1">
                    {indexarStatus.data.modo === 'indexing_api' ? (
                      <>
                        ...
                        <p>
                          Total: <strong>{String(indexarStatus.data.total)}</strong> URLs
                          &nbsp;|&nbsp; Indexadas:{' '}
                          <strong>{String(indexarStatus.data.indexados)}</strong> &nbsp;|&nbsp;
                          Erros:{' '}
                          <strong>{(indexarStatus.data.erros as unknown[])?.length ?? 0}</strong>
                        </p>
                      </>
                      ...
                      <>
                        ...
                        <p>
                          Google: {indexarStatus.data.google ? '✅' : '❌'} &nbsp;|&nbsp; Bing:{' '}
                          {indexarStatus.data.bing ? '✅' : '❌'}
                        </p>
                        <p className="text-xs opacity-75">
                          Sitemap: {String(indexarStatus.data.sitemap)}
                        </p>
                      </>
                    ) : (
            ...
            <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 text-sm text-brand-muted">
              <p className="font-semibold text-brand-text mb-2">
                📋 Como configurar a Indexing API
              </p>
              <ol className="space-y-1 text-xs list-decimal list-inside">
                ...
                <li>
                  Ative a <strong>Web Search Indexing API</strong>
                </li>
                <li>
                  Crie uma <strong>Service Account</strong> e gere uma chave JSON
                </li>
                <li>
                  No{' '}
                  <a
                  ...
                  , adicione o e-mail da Service Account como <strong>Owner</strong> do site
                </li>
    ...
    cashback: {
      keys: [
        'cashback_ativo',
        'cashback_percentual',
        'cashback_validade_dias',
        'cashback_min_resgate',
        'area_cliente_ativo',
        'area_cliente_titulo',
        'roleta_ativa',
        'roleta_min_cashback',
      ...
      content: (
        <div className="space-y-5">
          <SectionTitle description="Configure o portal de autoatendimento dos seus clientes">
            Portal do Cliente
          </SectionTitle>
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            <div>
              ...
              <p className="text-brand-muted text-xs">
                Clientes podem acessar via código único em{' '}
                <code className="text-brand-accent">/minha-area</code>
              </p>
            </div>
            <Toggle
              value={get('area_cliente_ativo') || 'true'}
              onChange={set('area_cliente_ativo')}
              label="Área ativa"
            />
          </div>
          <Field label="Título da área do cliente">
            <Input
              value={get('area_cliente_titulo')}
              onChange={set('area_cliente_titulo')}
              placeholder="Minha Área"
            />
          </Field>

          <Divider />
          <SectionTitle description="Os clientes ganham cashback a cada festa realizada">
          ...
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            <div>
              ...
              <p className="text-brand-muted text-xs">
                Credita automaticamente quando o evento é marcado como &ldquo;realizado&rdquo;
              </p>
            </div>
            <Toggle
              value={get('cashback_ativo') || 'true'}
              onChange={set('cashback_ativo')}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <Field label="Percentual de cashback (%)" hint="Ex: 5 = 5% do valor total da festa">
              <Input
                value={get('cashback_percentual')}
                onChange={set('cashback_percentual')}
                placeholder="5"
                type="number"
              />
            </Field>
            <Field
              ...
              hint="Ex: 20 = precisa de R$ 20 para resgatar"
            >
              <Input
                value={get('cashback_min_resgate')}
                onChange={set('cashback_min_resgate')}
                placeholder="20"
                type="number"
              />
            </Field>
            <Field label="Validade (dias)" hint="0 = sem expiração">
              <Input
                value={get('cashback_validade_dias')}
                onChange={set('cashback_validade_dias')}
                placeholder="365"
                type="number"
              />
            </Field>
          ...
          <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4 text-sm text-brand-muted space-y-2">
            ...
            <ul className="text-xs space-y-1 list-disc list-inside">
              <li>
                Quando um evento é marcado como <strong>realizado</strong>, o sistema credita
                automaticamente o cashback.
              </li>
              ...
              <li>
                O código de acesso é gerado automaticamente e pode ser enviado via WhatsApp na tela
                do cliente.
              </li>
            </ul>
          </div>
          ...
          <Divider />
          <SectionTitle description="Roleta de prêmios que o cliente pode girar ao acumular cashback suficiente">
            🎡 Roleta de Prêmios
          </SectionTitle>
          <div className="flex items-center justify-between p-4 bg-brand-surface-2 rounded-xl border border-brand-border">
            <div>
              ...
              <p className="text-brand-muted text-xs">
                Exibe a roleta na área do cliente quando o cashback acumulado atinge o mínimo
              </p>
            </div>
            <Toggle
              value={get('roleta_ativa') || 'false'}
              onChange={set('roleta_ativa')}
              label="Roleta ativa"
            />
          </div>
          <Field
            label="Cashback mínimo para ganhar 1 giro (R$)"
            hint="A cada múltiplo desse valor em cashback total, o cliente ganha 1 giro. Ex: 200 = a cada R$200 acumulados"
          >
            <Input
              value={get('roleta_min_cashback')}
              onChange={set('roleta_min_cashback')}
              placeholder="200"
              type="number"
            />
          </Field>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-brand-muted mb-3">
              Configure os prêmios e seus pesos de probabilidade. Mais peso = mais chance de sair.
            </p>
            <PremioEditor value={get('roleta_premios') || '[]'} onChange={set('roleta_premios')} />
          </div>
          ...
          <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-4 text-sm text-brand-muted space-y-2">
            ...
            <ul className="text-xs space-y-1 list-disc list-inside">
              <li>
                O cliente ganha 1 giro para cada R${get('roleta_min_cashback') || '200'} de cashback{' '}
                <strong>acumulado total</strong>.
              </li>
              <li>
                Os giros são calculados automaticamente:{' '}
                <code className="text-yellow-600">
                  floor(cashbackTotal / mínimo) − girosJáUsados
                </code>
                .
              </li>
              <li>
                O sorteio é ponderado pelo <strong>peso</strong> de cada prêmio.
              </li>
    ...
    sistema: {
      keys: [
        ...
        'email_admin',
        'email_notificacoes',
        'eventos_antecedencia_dias',
        'max_brinquedos_orcamento',
      ],
      content: (
        <div className="space-y-5">
          <SectionTitle description="Parâmetros operacionais do sistema de CRM e gestão">
            Alertas e SLA
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field
              label="SLA de Follow-up (horas)"
              hint="Horas sem interação para gerar alerta de atenção no lead"
            >
              <Input
                value={get('sla_followup_horas')}
                onChange={set('sla_followup_horas')}
                placeholder="24"
                type="number"
              />
            </Field>
            <Field
              label="Antecedência de eventos (dias)"
              hint="Dias antes do evento para alertas no dashboard"
            >
              <Input
                value={get('eventos_antecedencia_dias')}
                onChange={set('eventos_antecedencia_dias')}
                placeholder="7"
                type="number"
              />
            </Field>
            <Field
              label="Máx. brinquedos por orçamento"
              hint="Limite de itens no carrinho de cotação"
            >
              <Input
                value={get('max_brinquedos_orcamento')}
                onChange={set('max_brinquedos_orcamento')}
                placeholder="10"
                type="number"
              />
            </Field>
          ...
          <Divider />
          <SectionTitle description="E-mails para notificações automáticas do sistema">
            Notificações por E-mail
          </SectionTitle>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="E-mail do administrador">
              <Input
                value={get('email_admin')}
                onChange={set('email_admin')}
                placeholder="admin@twixeventos.com"
                type="email"
              />
            </Field>
            <Field label="E-mail de notificações" hint="Recebe alertas de novos leads e orçamentos">
              <Input
                value={get('email_notificacoes')}
                onChange={set('email_notificacoes')}
                placeholder="notificacoes@twixeventos.com"
                type="email"
              />
            </Field>
          </div>
          ...
          <div className="bg-brand-surface-2 rounded-xl border border-brand-border p-4 space-y-3">
            ...
            <p className="text-brand-muted text-xs">
              Para alterar a senha do admin, gere um novo hash:
            </p>
            <code className="block bg-brand-bg border border-brand-border rounded-lg p-3 text-xs text-green-600 font-mono">
              node -e
              &quot;require(&apos;bcryptjs&apos;).hash(&apos;novasenha&apos;,12).then(console.log)&quot;
            </code>
            <p className="text-brand-muted text-xs">
              Cole o hash gerado diretamente na tabela{' '}
              <code className="text-brand-text">admin_users</code> no NeonDB.
            </p>
          </div>
        </div>
      ),
