...
Por exemplo, se o prefixo for "O que é" e o nicho for "brinquedos infláveis e festas", para a letra "C", você DEVE sugerir termos exatamente no formato:
"O que é Cama Elástica", "O que é Castelo Inflável", "O que é Corrida de Obstáculos" (garantindo que a palavra-chave/termo principal do nicho após o prefixo comece estritamente com a letra "${uppercaseLetra}").`
      : `Não utilize nenhum prefixo. Gere apenas os termos simples começando estritamente com a letra "${uppercaseLetra}". Ex: "Cama Elástica", "Castelo Inflável", "Corrida de Obstáculos".`

    const prompt = `Você é um especialista em Marketing Digital, SEO local e criação de glossários corporativos altamente indexáveis no Google.
...
${promptExtra ? `Instruções adicionais importantes: ${promptExtra}` : ''}`
    ...
    if (!rawText || rawText.trim().length === 0) {
      throw new Error('A API do OpenRouter retornou uma resposta vazia. Por favor, tente novamente.')
    }
