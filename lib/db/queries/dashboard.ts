...
export async function getDashboardMetrics() {
  ...
  const [
    ...
    `).then(r => (r as unknown[])[0] as { nome: string; total: number } | undefined),
    ...
    `).then(r => r as unknown as { origem: string; total: number }[]),
    ...
    `).then(r => r as unknown as { nome: string; total: number }[]),
